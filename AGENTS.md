...
- Nodes 2.0 会把实现 `computeLayoutSize()` 的 DOM widget 作为可分配的 `auto` 行处理；只声明 `getMinHeight()` 仍会让宿主把节点新增高度分给该 widget，造成工具栏、列表或空状态随拉伸越排越开。需要“节点可继续增高、内容保持顶部紧凑”的 widget 必须同时声明稳定且相等的 `getMinHeight()` 与 `getMaxHeight()`，把额外高度留给节点主体。DOM widget 元素实际挂在独立 `.dom-widget` overlay 中，不是 `.lg-node-widgets` 的后代，禁止用业务 CSS 的 `:has(.业务根)` 试图反向覆盖宿主网格；此类尺寸问题必须通过 widget 布局协议修复，并以当前安装版本的 `domWidget.ts`、`useProcessedWidgets.ts` 和 `NodeWidgets.vue` 为依据。
