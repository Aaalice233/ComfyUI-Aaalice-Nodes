...
- 紧凑节点、Popover、设置页和 Dialog 默认只保留一个能说明当前区域的标题，禁止把“大标题 + 副标题”当作固定模板。只有会影响当前决策、风险理解或功能边界的内容才允许作为辅助说明，并应放在相关控件或操作附近；页面身份、标题同义改写、宣传文案和显而易见的操作解释必须删除。
- 空间不足时先删减说明和重复层级，不得通过把副标题、操作提示或关键说明缩成难以阅读的小字来塞进界面。正文与实质性说明使用正常可读字号；9px 以下只允许短 ID、计数或弱化元数据，不得承载用户必须理解的信息。
...
- [ ] 紧凑界面没有套用无必要的“标题 + 副标题”；实质性说明没有被压缩为难以阅读的小字
