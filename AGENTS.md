...
- 新增或修改任何包含用户可见界面的功能时，若当前环境提供 `product-design:index`，必须在设计和实现前优先加载，并按其路由使用适合当前任务的 Product Design 工作流；不得等功能代码完成后才临时补一层样式。Skill 不可用时继续依据本节与 [`ui-system.md`](docs/design/ui-system.md) 完成同等标准的设计，不得因此降低质量。
