...
- Dashboard 边界翻页监听必须挂在不参与页面位移动画的稳定舞台上，状态按真实侧栏根实例隔离，并在重挂、切换表面或销毁时解除监听和动画帧。连续滚轮事件不排队，每 180ms 最多产生一个翻页意图；停止后不追加翻页，禁止按 `deltaY` / `wheelDelta` 幅值猜测跨越页数。横向占优输入只刷新当前手势，不触发翻页；页面请求按当前 Page Id 解析，内部原生滚动、事件目标所在的嵌套滚动控件和停止传播的隔离控件优先消费输入，旧 post-scroll rAF 只检查最后一个越界方向且不得重放历史事件。手动选页、工作区/工作流切换、搜索、布局模式、网格或页面菜单拖拽开始必须同步取消当前手势；断连、隐藏或禁用的根不得继续翻页。多侧栏根分别捕获过渡快照，共享 Page Id 的同帧反向请求回到原页时折叠为无过渡 no-op；宿主移除、替换或隐藏自定义页签根时必须立即清理控制器及该根的锚定浮层、Tooltip 与 Context Menu，不得关闭其它根的菜单；首次挂载或持续 `v-show` 隐藏的根不得参与完整 Provider/控件重建，重新显示时补一次渲染，焦点请求只能由发起操作的可见根消费；多根生成的 DOM 控件 id 必须按根隔离，脱离根挂载的浮层必须记录显式 owner，控件销毁必须清除自己的浮动编辑器并闭合未完成手势。
