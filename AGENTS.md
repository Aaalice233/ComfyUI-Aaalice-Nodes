...
- Nodes 2.0 的 `useProcessedWidgets` 只把 `computeLayoutSize()` 归约为 `hasLayoutSize`，`NodeWidgets` 随后把该 widget 设为 `auto` 网格行，并在存在 `auto` 时让整个 widget stack `flex: 1`；它不会用 `getMinHeight()` / `getMaxHeight()` 生成 Vue 网格轨道。因此多个 DOM widget 会分享节点新增高度，造成工具栏、列表或空状态越拉越开。Classic 仍通过这两个高度参与 LiteGraph 分配；需要“节点可继续增高、内容保持顶部紧凑”的双模式节点，必须分别约束 Classic widget 高度和 Nodes 2.0 widget stack 的顶部内容轨道。`WidgetDOM.vue` 在 Nodes 2.0 会把业务元素挂进 `.lg-node-widgets`，Classic 才使用独立 `.dom-widget` overlay，禁止根据其中一种模式的 DOM 结构推断另一种。Nodes 2.0 原生缩放还只读取节点元素 inline `min-width`，不读取 computed CSS；业务最小宽度必须同步到真实节点元素并在重挂后恢复。上述均为当前前端内部契约，升级时重新核对 `domWidget.ts`、`useProcessedWidgets.ts`、`NodeWidgets.vue` 与 `useNodeResize.ts`。
...
- 纯前端界面、样式、布局和局部交互修改，若不涉及 slot、widget 尺寸协议、序列化、执行链、复杂 DOM 生命周期、子图、双模式或性能回归，完成代码检查后交给用户刷新现有 ComfyUI 页面验收；默认不启动独立实例、不打开 `agent_browser`，也不代替用户做 GUI 操作。
- 只有大功能开发或涉及复杂交互生命周期、DOM widget 重挂、子图 / Classic / Nodes 2.0 双模式、协议序列化、执行链、性能与资源行为的变更，才按 [`comfyui-agent-browser-testing`](C:/Users/Administrator/.pi/agent/skills/comfyui-agent-browser-testing/SKILL.md) skill 启动隔离实例并执行自动化 GUI 验收；用户当次明确要求自动验收时除外。
...
### 8.1 `agent_browser` 自动 GUI 验收

- 本节只适用于上一条明确列出的高风险大功能或用户明确要求自动验收的任务；普通纯前端界面改动不要进入本节，直接交用户刷新现有页面确认。
- 仅当当前会话提供可用且稳定的 native `agent_browser` 时执行；不可用或不稳定就交用户手测，不启动有窗口浏览器、不连接 CDP、不使用 Playwright / Selenium。
- 通用 ComfyUI 自动化流程统一阅读并执行可复用 skill：`C:/Users/Administrator/.pi/agent/skills/comfyui-agent-browser-testing/SKILL.md`。
- skill 的 `Invoke-ComfyUiTestRun.ps1` 负责隔离端口、`user-directory`、数据库、日志、工作流源文件副本、manifest、状态、停止和后端重启；不要在项目文档中复制另一套启动脚本。
- 本项目 GUI 验收仍必须覆盖风险匹配的最短闭环：工作流副本真实载入、目标节点/子图定位、Classic / Nodes 2.0（涉及双模式时）、目标交互、截图、Console / page errors 和源文件未被修改。
- 载入必须使用 ComfyUI GUI 的 `文件 → 打开` 与 `#comfy-file-input`；工作流中心、URL hash、API 或写入式 `eval` 不能作为载入证明。刷新后若回到 `Unsaved Workflow`，按 skill 重新上传副本。
- 前端改动使用 fresh 页面/硬刷新；后端、注册、路由或执行链改动使用 skill 的完整 `restart`，再 fresh 打开并重新载入副本；HotReload 不替代后端重启。
- 成功证据应包含上传结果、载入截图、只读图摘要和错误检查；临时端口、绝对路径、截图路径和一次性坐标不写入长期记忆。
