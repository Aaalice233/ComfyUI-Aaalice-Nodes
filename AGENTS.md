...
- 仅当当前会话提供可用且稳定的原生 `agent_browser` 时执行这套自动验收；不可用或连接不稳定就停止 GUI 自动化，改交用户手测，不自行启动有窗口的 Chrome / Edge，不连接 CDP，也不引入 Playwright / Selenium。
- 自动验收必须启动独立的父级 ComfyUI 实例，使用空闲的 loopback 端口；严禁占用用户日常使用的 `8188`。默认无头（后台、不显示浏览器窗口）运行，并记录、核对本轮实例的 PID、命令行和端口。
- 载入用户工作流前必须先把源 JSON 备份到本轮隔离目录，并优先载入备份副本进行交互；不得覆盖原工作流、用户未保存状态或日常实例。不能用空白工作流替代调用方提供的工作流。
- 前端修改后必须硬刷新或重开无头页面；后端修改、服务端注册变化或执行链变化必须完整停止并重新启动独立实例，再载入备份副本验收，不能把 HotReload 当作后端重启。
- 交互验收按改动风险选最短闭环：载入工作流后定位目标节点或子图，打开右键菜单，按需创建/进入子图并显示公开控件，再检查侧边栏、状态、截图和 Console；不为“全量覆盖”机械重复无关路径。
- 成功验收中可复用的流程和边界（例如工作流载入、子图创建、节点定位、右键菜单打开）可以更新到项目记忆；临时端口、绝对路径、截图路径、坐标和一次性故障不要写入长期规则或记忆。

#### 标准执行顺序

1. 确认当前会话确实能使用原生 `agent_browser`；不能使用时停止自动化，不改用有窗口浏览器。
2. 确认父级 ComfyUI 根目录和调用方提供的 `$WorkflowPath`；只读检查源文件存在，不能从 AGENTS 或脚本中猜测用户路径。
3. 创建本轮隔离运行目录，把源文件复制为 `workflow-source-backup.json` 和 `workflow-under-test.json`；所有交互只针对测试副本，原文件始终不写入。
4. 检查日常实例的 `8188` 不被占用或触碰，另选空闲 loopback 端口；以隔离 user directory、数据库和日志启动父级 ComfyUI，进程和浏览器都默认后台运行，并保存 PID、命令行、端口和日志位置。
5. 等待启动日志确认真实 GUI URL 与 web root，再用 `agent_browser` 打开该 URL；不能只根据启动命令或端口猜测服务已就绪。
6. 通过 GUI 的打开/上传入口载入 `workflow-under-test.json`，等待工作流恢复完成，并用页面状态、截图和针对性读取确认目标节点/子图已经出现；不得用 API 或写入式 `eval` 代替真实载入。
7. 先用 role、label、`data-*` 或可读标题定位侧栏和目标节点；Canvas 无语义入口时才使用当前会话中的坐标，不能把坐标写入长期规则。
8. 执行本次改动对应的最短交互闭环：需要时进入或创建子图，显示公开 widget，返回宿主节点，打开右键菜单，选择绑定参数入口，并检查侧边栏控件/对话框的实际状态。
9. 每个关键动作后读取针对性状态；涉及视觉时保存截图，同时检查 `agent_browser` 的 Console 和 page errors。只有目标行为和错误检查都通过，才报告 GUI 通过。
10. 改前端后硬刷新或重开同一无头页面，再载入测试副本并重复受影响闭环；若涉及 Classic / Nodes 2.0，分别切换真实节点渲染模式验证，不能把“画布模式”当作 Nodes 2.0。
11. 改后端、节点注册、服务端路由或执行链后，先核对并停止本轮 PID、确认端口已释放，再完整重启独立实例，重新载入测试副本并重复闭环；不得用 HotReload 代替重启。
12. 验收结束时再次核对 PID、命令行和端口后才停止测试实例；保留本轮日志和必要证据，确认源工作流未变化，并清理本轮创建的临时进程/资源。
13. 将成功流程中可迁移的步骤更新到项目记忆，将本轮路径、端口、坐标、截图和故障现场留在报告或日志中，不写入长期规则。
