...
- 在同一 ComfyUI 前端会话中切换工作流 Tab 后返回，Gallery 会重新加载当前逻辑页并恢复到原卡片及其视口相对位置；该临时锚点不会写入工作流或 Dashboard 预设。
...
- 网络错误横幅可点击查看、复制完整诊断并重试。Gallery 会在系统默认信任库基础上补充 `certifi` 公共 CA；出现 SSL 证书校验失败时，请检查系统时间、HTTPS 代理或抓包软件证书以及系统受信任根证书，插件不会关闭 HTTPS 证书校验。
...
- 悬浮预览会显示大图、分辨率、评分、收藏数、分级与分类标签；卡片与帖子详情都可下载原图，详情另提供在新标签页打开原图的入口；安装 ComfyUI-Autocomplete-Aaalice 后，搜索、排除标签与输出过滤等标签输入框获得其标签补全，悬浮预览和帖子详情同时显示标签翻译。
...
紧凑的预设选择器整体保存和切换看板的页面、布局组、绑定、卡片几何与兼容参数值，包括每个 Seed 的数值和执行后行为。列表中的预设可用左侧手柄拖动排序，也可聚焦手柄后使用 `Alt+方向键` 调整；顺序随工作流保存。常见模型参数（UNet、CLIP、VAE、Checkpoint、Upscale Model 等）在预设路径与本机目录结构不同时，会按文件名查找唯一的嵌套路径并在确认后应用；模型缺失或同名路径有歧义时会明确列出页面、组件和参数，同时仍写入新预设值，避免静默沿用上一个预设的模型。组管理组件只随预设切换各组的开关状态；颜色范围、排序、关闭模式和组联动关系始终由 Manager 节点共享。切换旧预设时，若基础参数值因历史接口错位而损坏，但当前工作流中的同一参数仍是有效值，系统会自动用当前值修复该预设并明确提示修复数量，无需逐项手动重存。本地修改后预设名称以斜体加末尾 `*` 标记；用 `Ctrl`/`Cmd`+`S` 保存工作流时也会同步提交当前修改到启用的预设。预设保存在工作流文件内，分享工作流（包括通过 Aaalice Workflow Hub）后接收方即可获得你随工作流携带的预设。


### 🎚️ 调整档案

工具栏的 **参数套用管理** 管理本机参数覆盖档案，支持导入、导出、比对预设生成规则和编辑推荐新预设名称。编辑自动保存在当前浏览器，不直接修改节点；在侧边栏预设列表选择 **复制**，再选择档案并确认套用，创建并切换到新副本，基础预设不变。工作流更新后，旧规则优先按稳定绑定匹配；身份变化时，只在 Provider、适配器、类型和参数名称一致且上下文可唯一确认时恢复。新增参数保留基础预设值，缺失、歧义、类型变化、无效选项和越界值会跳过并显示数量，不按位置硬套。联动规则整组校验，任一目标不可用则跳过整条规则；实际写入失败会回滚参数、布局和新副本。模型路径仅恢复唯一存在的本机路径，缺失或同名歧义模型不强制写入。没有可套用规则时不会创建空结果副本。档案不随工作流保存，可通过管理窗口的导出文件分享。

参数套用支持编辑全部内置可编辑控件类型，包括图像选择、LoRA 列表、分辨率、提示词选择、组开关和画廊参数；只读执行结果不属于可套用参数。画廊只显示参数编辑区，不显示图片列表，也不会在编辑档案时搜图。组规则逐组恢复并提示缺失、歧义和冲突；新捕获的整组开关能跟随组内成员变化，旧档案仅恢复已有成员。旧组 ID 变化后，新档案可按唯一组名恢复；重名时不猜测。
...
**词库**视图管理词条、任意层级的分类树、多归属收藏夹、标签和每条词条的一张预览图。分类支持搜索、折叠、拖拽与键盘跨层排序；安全删除会提升子分类并把直属词条设为未分类，也可显式删除整棵分支，但不会删除词条。已选词条可一次移动、导出或事务化删除，每条词条的完整提示词可一键复制。词库可将全部或当前筛选结果导出为带哈希资产的 v2 ZIP；现有平面 SQLite 词库、v1 ZIP 和旧版 `data.json + preview/` 会自动升级，旧分类名中的有效 `/` 路径会一次性转换为分类层级，同时保留原叶分类 ID、颜色和词条归属。上传或导入的过大静态预览图会自动缩放并压缩为适合界面展示的 WebP，不会因单张原图略大而阻断整份词库。传输上限 2 GiB，全程流式处理而非读入内存。
