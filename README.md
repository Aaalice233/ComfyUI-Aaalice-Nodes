...
- **独立图片**（默认）：每个 IMAGE 批次项都是独立样本，输出数量与输入图片数一致。适合 Qwen Image、Krea2 和普通批量出图。
- **视频帧**：把完整 IMAGE 批次作为一段有序视频交给三维/视频 VAE。适合 SeedVR2、Wan 等需要时间连续性的处理；二维图片 VAE 会明确报错，避免静默产生错误语义。

节点不按模型名称猜测模式，因为相同的三维 VAE 架构既可能用于图片也可能用于视频。它一次调用 ComfyUI 原生 VAE 编码，让核心继续负责显存估算、自动批处理和 OOM 分块回退；不提供容易破坏时间维度的 `per_batch`。替换 `VAE Encode Batched 🎥🅥🅗🅢` 时直接连接原有 `pixels` 和 `vae`，然后选择正确的输入模式即可。
...
- 悬浮预览会显示大图、分辨率、评分、收藏数、分级与分类标签；安装 ComfyUI-Autocomplete-Aaalice 后，搜索、排除标签与输出过滤等标签输入框获得其标签补全，悬浮预览和帖子详情同时显示标签翻译；安装 prompt-assistant 后，卡片可通过其视觉分析反推图像。
