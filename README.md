...
Gallery 可从节点右键菜单加入 Dashboard；节点与侧边栏是同一运行时的两个完整视图，搜索、筛选、分页、选择、排序与设置会实时同步。窄侧边栏使用双行紧凑工具栏，两个 Switcher 的当前项显示图标与文字、其余项仅显示图标；搜索默认收起，并用更小图像卡片优先保持至少三列。只读徽章显示该节点当前的启用、静音或绕过状态，不反向修改节点模式。
