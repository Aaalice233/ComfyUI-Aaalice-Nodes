...
- 打开“随机抽卡”后会在当前来源、查询、频道和分级范围内随机抽取图片；前端使用 Web Crypto 安全洗牌，并在当前节点会话内避免重复展示相同帖子，关闭模式或改变范围后重新开始。Danbooru 的 Rating 是免费元标签，`random:<limit>` 则会占用一个查询名额：0–1 个普通标签使用原生随机并覆盖完整匹配集，两个普通标签使用高性能随机真实页，最多覆盖当前排序下最新约 60,000 条结果。随机结果继续遵循本机内容黑名单。
...
**词库**视图管理词条、任意层级的分类树、多归属收藏夹、标签和每条词条的一张预览图。分类支持搜索、折叠、拖拽与键盘跨层排序；安全删除会提升子分类并把直属词条设为未分类，也可显式删除整棵分支，但不会删除词条。已选词条可一次移动、导出或事务化删除，每条词条的完整提示词可一键复制。词库可将全部或当前筛选结果导出为带哈希资产的 v2 ZIP；现有平面 SQLite 词库、v1 ZIP 和旧版 `data.json + preview/` 会自动升级，无需手工转换。传输上限 2 GiB，全程流式处理而非读入内存。
