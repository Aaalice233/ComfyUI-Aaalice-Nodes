...
工作流升级后导入旧备份时，优先使用“仅导入数值”：新版预设的布局和绑定保持不变，系统会恢复精确匹配以及可唯一确认的同语义参数，歧义、失效或不兼容值会列出并跳过。若“布局和数值”会带回失效绑定，确认前会明确警告并推荐改用仅数值导入。切换预设时，无法恢复的值会按页面、布局组、组件和参数名列出；模型选项未出现在当前列表时，会提示检查模型名称与相对路径，文件确实存在则刷新 ComfyUI 网页后重试。

### 🎚️ 调整档案

工具栏的 **调整档案** 按钮打开保存在本机 ComfyUI 的全局参数覆盖档案。每条规则通过专用搜索选择界面选定一个侧边栏组件卡片并记录目标值（含 Seed 的执行后行为），多绑一卡片的联动目标会随主目标一起写入；候选覆盖侧边栏所有页面并标注所在页面，已有规则的组件不再重复出现。应用档案会在同一个事务中写入所有可匹配规则，失败时整体回滚。无法匹配或校验的规则会列入复核清单，不会静默跳过。档案不随工作流保存，因此无论打开哪份工作流都可以使用自己的档案。
