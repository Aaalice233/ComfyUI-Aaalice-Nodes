...
After upgrading a workflow, prefer **Values only** when importing an older backup. The new preset keeps its layout and bindings; exact matches and uniquely identifiable semantic matches are restored, while ambiguous, broken, or incompatible values are listed and skipped. If **Layout + values** would restore broken bindings, the confirmation explicitly warns you and recommends Values only. When switching presets, values that cannot be restored are listed by page, layout group, component, and parameter. If a model option is absent from the current list, the dialog asks you to check its name and relative path, then refresh the ComfyUI page and retry when the file already exists.

### 🎚️ Adjustment profiles

The toolbar's **Adjustment profiles** button opens value-override profiles stored globally on this ComfyUI installation. Each rule targets one sidebar component card through a dedicated searchable picker and remembers a target value (including a Seed's after-generate mode); linked targets of a multi-bound card are written together with the primary. Candidates span every sidebar page with a page badge, and components that already have a rule are hidden. Applying a profile writes every matching rule into the current controls in one transaction that rolls back on failure. Rules that cannot be matched or validated are listed for review instead of being silently skipped. Profiles are not embedded in workflows, so your own profiles stay available no matter which workflow is open.
