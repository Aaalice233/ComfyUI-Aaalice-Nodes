...
After upgrading a workflow, prefer **Values only** when importing an older backup. The new preset keeps its layout and bindings; exact matches and uniquely identifiable semantic matches are restored, while ambiguous, broken, or incompatible values are listed and skipped. If **Layout + values** would restore broken bindings, the confirmation explicitly warns you and recommends Values only.
