...
- When you switch workflow tabs and return within the same ComfyUI frontend session, Gallery reloads the current logical page and restores the same card and relative viewport position. This temporary anchor is not written to the workflow or Dashboard presets.
...
- Click a network-error banner to inspect and copy the complete diagnostic or retry. Gallery augments the system-default trust store with the public `certifi` CA bundle; if SSL certificate verification still fails, check the system clock, HTTPS proxy or packet-inspection certificate, and trusted root certificates. The extension never disables HTTPS certificate verification.
...
Every execution reads the visible composite of Krita's active document as `IMAGE`, the current selection as a same-size `MASK` (fully black when nothing is selected), and `METADATA` from the original PNG, JPEG, or WebP opened by that document. An unsaved document, a `.kra` document, or a source without generation parameters produces explicit empty metadata. Starting a queue first refreshes the Bridge and active-document status shown by every FetchFromKrita node; the actual image is still requested when the node executes and never reuses the status preview or a previous result. For first-time setup, close Krita, open **ComfyUI Settings → Aaalice Nodes → Krita** to install and enable the bundled `Aaalice Comfy Bridge`, then start Krita and test the connection.

Krita, ComfyUI, and the Bridge must run on the same machine. After the first installation, the Bridge updates with this package automatically: ComfyUI uses a cross-process lock to atomically replace an older version at startup or status refresh and restores the previous installation if replacement fails. If the operating system also blocks recovery, it preserves the recovery files and reports their path. It never overwrites an unknown newer version. If Krita is already running, restart it once when the node asks so the updated Bridge loads. The Bridge acknowledges each request before waiting for Krita to finish projection recomposition and export, so a complex document is not mistaken for an unresponsive Bridge. A missing Bridge, offline Krita, no active document, damaged source metadata, or export failure fails explicitly; the node never returns an old snapshot or placeholder.
...
The compact preset picker saves and switches the complete dashboard — pages, groups, bindings, card geometry, and compatible values, including each Seed's value and after-generate mode. Presets can be reordered with the handle on the left or, while that handle is focused, with `Alt+Arrow`; the order is saved with the workflow. When common model parameters (UNet, CLIP, VAE, Checkpoint, Upscale Model, and similar controls) use a different local folder structure, the preset finds a unique nested path by filename and applies it after confirmation. Missing or ambiguous model files are reported with their page, component, and parameter, while the new preset value is still written so a component cannot silently retain the previous preset's model. Quick Group Manager controls store only each group's on/off state in a preset; color scope, ordering, off mode, and linkage rules remain shared on the Manager node. When an older preset contains a damaged primitive value from a historical interface mismatch but the same live workflow control still has a valid value, switching automatically repairs that preset from the live value and reports the repair count instead of requiring each value to be resaved manually. Local changes mark the preset name in italics with a trailing `*`; saving the workflow with `Ctrl`/`Cmd`+`S` also commits the working copy into the active preset. Presets are stored inside the workflow file, so recipients of a shared workflow (including through Aaalice Workflow Hub) get the presets you shipped.


### 🎚️ Adjustment profiles

The toolbar's **Override profile management** manages browser-local parameter overrides, including import, export, preset comparison and a suggested name for new copies. Edits save locally without changing nodes. Choose **Duplicate** in the sidebar preset list, select a profile, and apply it to a new copy; the base preset stays unchanged. After workflow updates, rules first match stable bindings. Changed identities recover only when provider, adapter, type and control name agree and context identifies one target. New parameters keep their base values. Missing, ambiguous, incompatible or invalid rules are skipped with explicit counts, never matched by position. Linked rules are validated as a whole; an unavailable member skips the entire rule. Write failures roll back values, layout and the new copy. Model paths recover only to uniquely available local files; missing or ambiguous models are not forced. No copy is created when no rule can apply. Profiles are not embedded in workflows; share them using the manager's export action.

Override profiles support every built-in editable control type, including image selection, LoRA lists, resolution, prompt selections, group switches and Gallery parameters. Read-only execution results are not override parameters. Gallery shows parameter fields without an image list or search requests during editing. Groups recover independently with missing, ambiguous and conflicting states reported. Newly captured whole-group switches follow membership changes; older profiles restore only saved members. New profiles can recover changed group IDs by a unique group name, never by guessing between duplicates.
...
The **Library** view manages entries, an arbitrarily deep category tree, multi-membership favorite folders, tags, and one preview image per entry. Categories support search, collapse, pointer drag, and keyboard hierarchy moves. Safe deletion promotes children and uncategorizes direct entries; explicitly deleting a whole branch still never deletes entries. Selected entries can be moved, exported, or deleted in one transaction, and each entry's full prompt can be copied with one click. The library exports the whole collection or current filter as a v2 ZIP with hashed assets. Existing flat SQLite libraries, v1 ZIP files, and legacy `data.json + preview/` exports upgrade automatically: valid `/` paths in legacy category names become hierarchy once while preserving leaf category IDs, colors, and entry assignments. Oversized static previews from uploads or imports are automatically resized and compressed to display-ready WebP instead of blocking the whole library because one source image is slightly large. Transfers are limited to 2 GiB and streamed instead of loaded into memory.
