...
- Enable **Random draw** to sample the current source, query, feed, and ratings. The frontend uses Web Crypto to shuffle safely and avoids replaying the same post within the current node session; leaving the mode or changing that scope starts a fresh session. Danbooru ratings are free metatags, while `random:<limit>` consumes one query slot: searches with zero or one ordinary tag use native random sampling across the complete matching set, while two-tag searches sample high-performance real result pages covering at most the latest approximately 60,000 matches in the current order. Random results still respect the local content blacklist.
