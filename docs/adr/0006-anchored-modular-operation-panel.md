...
Operation Panel therefore uses `Panel → Page → Module`. Only Root Modules use UMG-style anchors against a saved design size. Node Card contents and child Modules inside containers continue to use automatic layout. The author may align and snap Root Modules to a 12-column guide and an 8px fine grid, but those guides are not serialized cells. Module height is content-driven, overlap is forbidden, and viewports narrower than the design width scroll horizontally instead of producing a second automatic layout.
