...
15. Dashboard 参数调整档案 Dialog 只编排本机档案状态与现有规则 codec：复制档案深复制全部规则并分配新身份，命名确认与 localStorage 持久化成功后才选中新副本，来源档案和当前侧边栏参数保持不变；规则列表和候选选择面互斥挂载，切换时保留规则滚动位置；规则按真实宿主来源分组并用稳定 Control Tone 着色，同名规则只增加组内序号，不把来源标题或页面写进持久身份；规则搜索只切换既有组/行的可见性。视觉与响应式布局由 `theme-value-profiles.css` 负责，不改变 `localStorage` schema、匹配策略或事务化应用边界。
