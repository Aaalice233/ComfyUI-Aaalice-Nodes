...
| Dashboard 参数调整档案 | 浏览器 `localStorage`（`aaalice.workspace.valueProfiles`） | 档案列表与规则（稳定 Binding Key、快照式目标值 payload、回退匹配用的控件名与宿主标题）以及档案级页面范围（`pages`，null 表示全部页面） | 工作流 JSON、节点属性、布局与选区状态 |
