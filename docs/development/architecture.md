...
| BooruGalleryNode | `node.properties.booruGalleryState`（查询上下文、逻辑页码、随机/选择模式、选择快照与 Dashboard 搜索展开状态）、节点自身 `mode`；Dashboard 值预设另以版本化整体 payload 保存该状态快照 | 单个控制器的搜索 Summary、随机会话已见集合、详情、当前请求、执行 payload、任意数量节点 / Dashboard Surface 投影，以及按 workflow / graph / node 身份隔离的有界会话视口锚点；Dashboard 只读显示节点 mode，并独立投影双行紧凑工具栏、共享 `activeLabelOnly` 收缩 Switcher 与更小瀑布流卡片 | 任一投影 DOM、搜索结果副本、随机顺序、cursor、滚动像素、Hover、Dialog、凭据或持久缓存 |
...
| DIY 侧边栏预设 | `app.graph.extra.aaaliceSidebarPresets` | 有序预设集合、当前完整 Dashboard 快照、控件值与基准预设身份 | 滚动、选区、编辑模式、图钉、搜索、词库与工作流节点结构 |
| Dashboard 参数调整档案 | 浏览器 `localStorage`（`aaalice.workspace.valueProfiles`） | 档案列表与规则（独立档案身份、卡片主绑定的稳定 Binding Key、快照式目标值 payload、回退匹配用的控件名与宿主标题） | 工作流 JSON、节点属性、布局与选区状态 |
...
3. 独立虚拟瀑布流按最短列增量放置，ResizeObserver 只观察容器；滚动由单一 rAF 计算可见区并差量挂载卡片，离开 overscan 的图片移除 `src`。滚动帧不重写未变化的卡片样式，布局对象以 revision 标记真实几何变化（`setItems`、`reflow`、列数变化），仅 revision 变化时全量同步已挂载卡片的宽高与 transform；卡片操作网格只消费这份既有 placement 宽高与当前可见操作数，完整网格放不下时才投影单一更多菜单入口，不安装卡片级观察器。可见项上报只在可见集合变化时触发，卡片倾斜动效由容器统一委托（单监听、单 rAF、每帧至多一次 `getBoundingClientRect`）。控制器只为当前用户交互拥有的活动投影保存会话级 `source + postId` 视口锚点及卡片相对偏移，也只接受该 owner 的可见项页码上报；其它同时活动的 Dashboard 投影不能覆盖共享逻辑页。节点挂载或恢复时从其真实 `node.graph` 捕获根工作流、所属图和节点 Id；活动投影让渡、滚动稳定、Surface 销毁和节点移除前都以这份身份把最新锚点复制到隔离的有界进程内 LRU。这样即使宿主先把根图 Id 清为零值，或清理 Subgraph 定义时不再逐个触发内部节点移除生命周期，也已有可恢复快照；工作流 Tab 返回并重新取得相同查询上下文和逻辑页 Summary 后把锚点交给新控制器一次性恢复。随机抽取、查询上下文或页码不一致时不复用旧锚点。Surface 隐藏、节点 / Dashboard 投影切换、Dashboard 重挂、工作流 Tab 往返和列数重排都在恢复锚点后再启用可见项回调，原始滚动像素不进入工作流或预设。暂停投影只记录自然尺寸失效，重新激活时合并为一次 reflow，不为每张图片在后台重复全量布局。快速滚动期间新挂载卡片只占位、滚动停止后统一补挂图片源，sample 预取防抖到停止后，滚动跨页的图 dirty 信号（`graph.change()`，会强制全画布重绘）合并到停止后。
...
8. 侧边栏预设纯模型保存有序预设集合、完整 Dashboard 与按稳定 Binding Key 索引的可序列化参数 payload；拖拽或键盘排序只移动集合项，不改写预设快照、基准身份或当前 Working Copy，并在一个 graph history 边界内写回工作流。模型从当前 Working Copy 与基准快照计算“已修改”状态；不存在基准时界面只显示中性占位。运行时协调器负责去重、捕获、预检以及布局与参数的共同应用和失败回滚；捕获的 live payload 必须先通过 Provider 预设 codec，失败时保留旧快照且不把未变化的基准标成 `attention`；该值若在实际预设应用时仍无效，再进入预检问题。普通控件以自身当前值为真源；Subgraph promoted widget 的日常读值、预设捕获与比较保留每个 wrapper 独立的宿主投影，原生布尔投影在 Provider 边界把 ComfyUI 的合法 `0/1` 表示规范化为 `false/true`，避免保存后被 codec 反复判为异常。内部预设切换若发现基础 payload 类型损坏，仅在同一 Binding 的可信 live payload 可读取且通过当前 codec 时，以该值修复目标预设并从复核问题中移除；此时 promoted widget 才沿公开提升链读取最深层真实 source widget 作为恢复依据，并把修复值写回当前 wrapper 的宿主投影；修复随同基准切换在同一图事务提交；约束变化、选项失效、解析或 codec 错误仍保留为显式问题。预检问题展示从快照布局反查页面、布局组、组件显示名和参数名，并把已知 reason code 转为可行动说明。模型文件型 `missing-option` 在运行时边界先规范化 `/` 与 `\\`，再按文件名检索当前 Combo：唯一候选进入用户确认并只为本次应用替换为实际嵌套路径；无候选或多个同名候选进入明确诊断，但仍把新预设的模型值加入原子写入计划，禁止静默保留旧预设模型。预设快照本身不改写为接收方的本机路径。禁止向用户显示 Binding Key、promoted 元组或内部状态码。跨工作流参数覆盖使用独立参数套用档案，侧边栏完整预设不再提供独立 JSON 文件导入导出。工作区入口负责 ComfyUI 图事务、对话框、切换保护和工作流序列化，Provider 继续是唯一写回节点的边界。预设集合与基准身份位于 `app.graph.extra.aaaliceSidebarPresets`，随工作流文件分发（含 Workflow Hub 的打包与安装，该插件原样保留 `extra`），跨插件契约是“不得剥离未知 extra 键”，Hub 侧零耦合。图同步签名同时覆盖看板与预设 extra，结构相同但持久状态不同的工作流切换标签页时必须刷新。预设保存规范化后的 V4 整数跨度；Provider 的运行时投影不进入持久快照。
...
15. 参数套用档案保存在浏览器 localStorage，不进入工作流；管理界面只编辑、导入和导出规则，复制侧边栏预设是唯一套用入口。预览和确认共用 value_profile_application 的规划逻辑，以所选基础预设的 Dashboard 为目标，不使用当前可见侧边栏或旧档案布局。先按稳定 Binding Key 匹配；Key 变化时仅在 Provider、Adapter、值类型及规范化参数名一致的候选中，按同宿主、宿主标题或控件标签提供约束，候选必须唯一，多个规则争用目标时精确身份优先，其余歧义跳过。预检类型、可用性、值域与 codec，模型只接受真实可用的唯一路径；联动规则整卡校验，重叠写入冲突不得按顺序覆盖。新增参数保留基础值，旧规则未匹配的 Key 不写入副本；未套用规则保留在本机档案并明确显示跳过状态和数量。确认重新规划，只要有可用规则就创建副本并原子切换；零匹配不创建副本，写入失败回滚参数、布局、基准和副本。首次空列表也提供导入入口，重名与持久化失败保留对话框和输入。
