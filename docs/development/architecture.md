...
14. Dashboard 布局交互状态只存在于 `dashboard_interactions.js`：选择集合由工作区持有，完整命中的 Layout Group 规范化为根级单元；拖拽按选区规范占位生成前缘插入标记，并用与 `dashboard_layout.insertEntries()` 相同的稳定顺序预演碰撞链下推。自动滚动使用单一 `requestAnimationFrame` 循环，在同一指针坐标下重算框选和落点；取消或提交清理预览、临时 transform 和组接收态，只有松手或键盘微调进入一次 Dashboard 命令。控件卡片的高矮宽窄适配完全由 `theme-dashboard-layout.css` 的尺寸 Container Query 派生：不读取 Provider、不安装每卡 ResizeObserver、不重建 Control View，也不写回规范网格几何。
