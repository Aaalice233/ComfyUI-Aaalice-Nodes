...
- 共享图像选择器的栅格图片和紧凑字段只请求 `nodes/tools/image_thumbnail_routes.py` 提供的 256px WebP，SVG 保持原样响应；路由以 ComfyUI `input` / `output` / `temp` 目录和 containment helper 重新验证路径。`blake3:` Asset 缩略图由前端经 `api.fetchApi()` 携带当前用户身份，再以可撤销 Blob URL 挂载；编码结果按源文件 stat 有界缓存，同一文件的并发请求去重且最多同时解码两张。只有用户打开悬浮大图时才请求原始 `/view` 资源。
