...
| BooruGalleryNode | `node.properties.booruGalleryState`（查询上下文、逻辑页码、随机/选择模式与选择快照）与节点自身 `mode` | 单个控制器的搜索 Summary、随机会话已见集合、详情、当前请求、执行 payload，以及任意数量节点 / Dashboard Surface 投影；Dashboard 只读显示节点 mode，并独立投影双行紧凑工具栏、共享 `activeLabelOnly` 收缩 Switcher 与更小瀑布流卡片 | 任一投影 DOM、搜索结果副本、随机顺序、cursor、滚动像素、Hover、Dialog、凭据或缓存 |
