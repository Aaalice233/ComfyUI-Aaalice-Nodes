...
10. Dashboard 边界翻页由 `dashboard_interactions.js` 的独立控制器拥有。每个已挂载侧栏根使用独立状态；稳定 Page Stage 接收滚轮，页面 Scroll Surface 只提供实时边界。连续滚轮事件不排队，每 180 ms 最多生成一个翻页意图；不按 delta 幅值猜测跨页，停止后不追加翻页；页面请求按当前 `activePageId` 解析，多根同帧回到原 Page Id 时折叠为无过渡 no-op。横向占优输入只刷新当前手势，不触发翻页；同一斜向手势中的横向占优脉冲（包括 `deltaY === 0` 的纯横向帧）只刷新当前手势，不触发翻页。事件目标所在的嵌套滚动控件和页面原生滚动优先消费输入；post-scroll rAF 只保留最后一个越界方向，不重放旧滚轮事件。手动选页、页面首尾、工作区/工作流切换、搜索、布局模式、网格或页面菜单拖拽开始和宿主卸载同步取消当前手势；断连、被 `v-show` 隐藏或禁用的舞台不得继续翻页；多 Graph View 共享当前工作区但分别持有控制器和过渡快照，重挂按实时 Page Id 确认请求；`v-show` 隐藏根休眠且不重建 Provider/控件，首次挂载已隐藏的根只建立空占位与生命周期观察，重新可见时由 ResizeObserver 补一次完整渲染，搜索焦点只由发起操作的可见根消费；每个根生成独立 DOM 控件 id，禁止跨根 `label[for]` 命中。自定义页签宿主被 Vue 或其它 custom renderer 替换时，由根与父级所有权观察器清理旧实例及其挂到 `document.body` 的锚定浮层；销毁、隐藏与重绘只关闭锚定于对应根的 Tooltip、Popover 和 Context Menu，不得关闭其它 Graph View、画布节点或扩展的表面；所有脱离根挂到 `document.body` 的菜单必须保存显式 owner，控件销毁同时移除自身浮动编辑器并闭合尚未提交的图事务。
