...
7. 随机模式只把开关写入 `booruGalleryState`；已见 `source + postId` 集合由单个前端节点会话拥有，并以来源、查询、频道、周期和 Rating 组成作用域，退出模式或作用域变化时释放。随机 API 不接收顺序 cursor、不写搜索结果缓存，前端请求同时使用 `cache: "no-store"`。Danbooru 的 Rating 是免费元标签，`random:<limit>` 会占用一个公开查询名额：0–1 个普通标签使用原生随机覆盖完整匹配集，不执行 `order:random` 全量排序；两个普通标签改走 `/counts/posts.json` 加普通 `latest` 页，计数查询与结果查询使用相同 Rating 和认证参数，计数缓存键包含来源、规范化查询、规范化 Rating、页大小及账户身份，随机页以 `secrets.randbelow()` 从最多 1000 个数字页中无偏选择。双标签默认每页 60 条时只覆盖当前排序下最新约 60,000 条；总数为 0 仍请求第 1 页，错误计数结构必须带端点和查询上下文显式失败。Gelbooru / Safebooru 搜索使用来源原生随机排序，AI TAG 以只缓存总数的随机页采样覆盖全集，前端再用 Web Crypto 拒绝采样驱动 Fisher–Yates 洗牌并做稳定身份去重。适配器继续先执行本机黑名单与必要的本地 Rating 过滤，客户端只把实际展示项记入已见集合；唯一结果耗尽采用有限空批次预算，不以无限翻页维持表面成功。
