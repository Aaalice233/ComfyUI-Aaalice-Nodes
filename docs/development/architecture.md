...
4. 用户开关先在纯模型中规划同 Manager 级联和节点模式变化；现存来源组的环路、缺失目标、路径冲突或重叠组冲突会在写入前中止。已删除来源组遗留的规则继续保存在工作流中，以便同 Id 的组恢复后检查，但不会阻断其它现存组编辑联动。节点面和侧边栏都通过同一 runtime 提交模式，并在图事务完成后刷新另一表面。
