...
- QuickGroupManager 作为一个整体控件添加到侧边栏时，卡片使用稳定 `quick-group-manager` Binding，展示当前 graph 的组、Mute / Bypass 与状态，开关必须复用 runtime 的级联预检和单次 graph transaction；参数面板、侧边栏和节点读取同一 Manager 状态。侧边栏预设覆盖 Manager 配置及组节点模式，保存/加载、复制、撤销/重做、缺失节点和类型不匹配均明确处理，不引入独立聚合页或轮询。
- 所有 Dashboard 控件只能使用尺寸目录中的 3/6/9/12 列宽与 13/18/28/36 行高；Provider 的任意历史跨度必须在模型边界向上吸附，拖拽和键盘缩放只能在同一列表内移动。回归覆盖旧布局迁移、来源同步、预设往返、窄栏投影、组内/页面级碰撞和 Classic / Nodes 2.0 的实际对齐。
