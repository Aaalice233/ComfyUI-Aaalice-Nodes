...
- ParameterPanel 的侧边栏投影必须覆盖无 Separator、根分区、单个与多个 Separator、开头/连续/末尾空分区、单参数分区和 Separator 改名。添加 Dialog 按原顺序显示非空分区；自动组使用稳定 Separator Parameter Id 区分同名分区，新建组成员从组内左上角按原顺序紧凑排布，并创建对应的 Dashboard 分隔项。普通显示不重复绘制同名组头，编辑模式仍可操作组头；后续新增复用完全匹配的组与分隔项并保留用户组名，且不得把分区卡片归入同面板的旧无作用域来源组或重排既有卡片。
