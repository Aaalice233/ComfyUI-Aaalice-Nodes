...
- 连续滚动到至少第 5 个逻辑页后，浏览器页签隐藏/恢复、节点与 Dashboard 投影来回切换、Dashboard Surface 销毁重建、工作流 Tab 往返以及改变容器宽度/列数，都必须保留同一 `source + postId` 卡片及其视口相对偏移，页码不得回到第 1 页；跨工作流回归必须证明锚点按 workflow / graph / node 身份隔离，覆盖滚动稳定与 Surface 销毁快照、ComfyUI 在节点移除回调前先把根图 Id 清为零值的顺序，以及根图清理未逐个触发 Subgraph 内部节点移除生命周期的路径；重新请求匹配的逻辑页后才恢复，不进入工作流 JSON 或 Dashboard 预设，随机抽取、查询上下文变化或显式跳页不得复用旧锚点。多个 Dashboard 根同时活动时只有用户正在交互的 owner 可以同步逻辑页，后台投影的重排不得覆盖页码；暂停投影收到多次图片自然尺寸修正时只记失效，重新激活后合并为一次 reflow。
- 跨页多选、稳定去重、拖动排序、本地标签编辑与恢复、保存加载、复制、撤销重做和连续 Queue 独立快照。
- 并发原图下载后恢复选择顺序；同一 Prompt 内的多个 Gallery 节点共享最多三路原图下载，同一进程连续执行两次包含四张以上选择的 Prompt 仍必须成功，不得跨事件循环复用执行期信号量。任一图片下载、Content-Type、大小或解码失败时整节点失败，不跳项、不补黑图。媒体代理必须保留适配器声明的站点请求头并在重定向后继续复核 URL；Gelbooru 缩略图、Sample 与 Original 的真实下载均须携带站点 Referer，验证返回 `image/*` 而不是 HTML 帖子页。
...
- 图像控件必须物化同一个 32px 共享控件：点击选择、文件拖入、拖放反馈、裁切缩略图、即时完整预览和清空交互保持一致。原生图像 Combo 还需分别覆盖 `input`、`output`、`temp` 目录、显式 `[type]` 标记和经认证请求按当前用户解析的 `blake3:` Asset 引用；栅格卡片及紧凑字段只请求不超过 256px 的 WebP，SVG 与悬浮大图仍请求真实原图。使用 20–30MB 图像密集滚动时不得在浏览器解码原尺寸卡片纹理，滚动中不启动新缩略图请求；EXIF 转置只能接收已缩小图像，同源并发请求只编码一次且后台最多同时解码两张，覆盖源文件后 stat 变化必须生成新缩略图。Asset Blob URL 在加载、失败、替换和虚拟卡片卸载路径均需释放。缩略图端点对非法类型、越界路径和非图像文件返回明确 HTTP 错误；上传非图像文件或收到无文件名响应时必须显示明确错误。
...
- 参数调整档案的顶部操作覆盖新建、复制为新档案、重命名和删除；复制时默认建议不冲突的“原名称 + 副本”名称，确认后完整复制全部规则并立即选中新身份，后续编辑不与来源档案共享 payload 引用，来源内容和当前侧边栏参数都不变化；命名取消、重名以及 localStorage 写入失败时不得留下半成品或切换当前档案。窄宽度下四个操作按钮不得遮挡档案选择器、待处理徽标或名称。
- Dashboard 顶栏固定为页面身份/预设/设置行与操作行两行，预设选择位于第一行页面设置按钮左侧，长页面名或 Layout Group 名称使用省略且不得使 Page Rail、Scroll Surface 或操作行在一行/两行之间跳变；窄栏时操作行只在自身横向滚动。完整侧边栏预设在普通模式和布局模式顶栏创建、应用、保存修改、另存、放弃修改、复制、重命名和删除；多预设列表的专用手柄常态必须显示可辨识的 tonal 胶囊表面和抓取光标，并完整覆盖 Hover / Focus / Active / dragging 视觉状态；可从手柄拖拽排序，也可在手柄上使用 `Alt+ArrowUp` / `Alt+ArrowDown` 逐项移动，移动后弹层不关闭、焦点不丢失，基准身份和预设内容不变，工作流保存/加载与撤销/重做保持顺序；便携 JSON 导出将当前选中预设名称写入 JSON 的 `name` 字段并作为文件名；导入优先使用该名称，同名时自动建议不冲突的名称，且在同一事务中创建、应用和选中新预设。多页面重复 Binding 只保存和写入一次。布局或参数偏离基准后保留预设名称，以斜体和末尾 `*` 表示修改，没有基准时显示中性占位的“选择预设”，同时说明差异数量；切换前覆盖保存/另存、放弃与取消路径。
- 缺失、移出侧边栏、类型变化、选项失效和第三方 codec 拒绝值均进入复核；确认后保留失效卡片并应用兼容内容，任一写入失败同时回滚布局和全部已写控件。捕获预设值必须先经过当前 Provider codec 校验，错误类型的 live payload 不得覆盖已有保存值；当前环境缺少模型下拉选项等原因导致 live payload 校验失败时，只保留原保存值，不得把未变化的基准预设标成 `attention`，选项失效仍在实际切换或应用该预设时进入复核；切换内部预设时，旧快照中基础 payload 类型损坏且可信 live 值可读取、可校验的条目自动以当前值修复并持久化到目标预设，不弹复核框但以 Toast 明示数量；Subgraph promoted widget 必须覆盖“宿主投影值因接口顺序变化而错位、内部 source widget 值仍正确”的真实恢复路径，同时覆盖 ComfyUI 原生布尔宿主值以 `0/1` 保存的正常路径：日常适配、预设捕获和基准比较应保留 wrapper 独立值并把 `0/1` 规范化为布尔，合法值不得产生无法清除的 `attention`；只有损坏值恢复才读取 source widget，当前 source 值同样无效、读取失败、约束变化或选项失效时仍复核。内部预设和便携 JSON 共用快照协议；外部导入两种模式均输入名称并创建新预设，不覆盖现有预设。
