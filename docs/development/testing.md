...
- 侧边栏与 ParameterPanel 图像参数必须物化同一个 32px 共享控件：点击选择、文件拖入、拖放反馈、裁切缩略图、即时完整预览和清空交互保持一致；清空或上传后同步两处状态。原生图像 Combo 还需分别覆盖 `input`、`output`、`temp` 目录和显式 `[type]` 标记，缩略图与悬浮大图必须请求真实目录。非图像文件和无文件名响应必须显示明确错误。
