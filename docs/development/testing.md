...
- “⚡ 快速组”页面与 Controls、Groups、Library 并列；根图、嵌套 Subgraph 和共享 Subgraph 定义中的每个 QuickGroupManager 只出现一次并按所属 graph 分卡。卡片显示作用域、组数、Mute / Bypass、颜色、状态、节点数，组开关必须复用 runtime 的级联预检和单次 graph transaction；定位先切换到所属 graph。搜索只影响当前会话，空状态、窄侧栏、键盘焦点、Classic / Nodes 2.0 标题与滚动均正确，页面和节点不引入轮询。
