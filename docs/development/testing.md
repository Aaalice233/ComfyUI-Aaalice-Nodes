...
- 侧边栏调参时只定向更新对应控件，不重建节点面或整个工作区；搜索为空提示严格遵守 `hidden`，存在可见控件且未搜索时不能占据页面底部。Dashboard 搜索输入非空时覆盖全部页面，仅匹配参数组件显示标题，不匹配页面名、布局组名或页面上下文；结果按页面分组显示，点击结果跳转到目标页面并定位组件，清空查询后恢复当前页原布局。
