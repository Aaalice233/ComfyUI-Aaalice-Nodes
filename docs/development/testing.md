...
- 参数预设在普通模式顶栏创建、应用、保存、恢复、复制、重命名和删除；多页面重复 Binding 只保存和写入一次。当前值偏离已应用预设时显示“已修改”，切换前覆盖保存、放弃与取消三条路径；缺失、移出侧边栏、类型变化、选项失效和第三方 codec 拒绝值均进入复核，兼容值一次提交，任一写入失败回滚全部已写值。
- 参数预设集合、最近应用身份和原节点值分别覆盖保存、加载、撤销/重做和工作流切换；创建、更新、删除和应用后出现保存工作流提示。参数预设与布局备份文案及状态键保持独立，导入布局不能覆盖工作流已有参数预设集合。
