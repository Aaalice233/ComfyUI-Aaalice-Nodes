...
- Dashboard V3 控件宽度覆盖 3–12 的每个整数列跨度，行高覆盖不小于 13 的任意整数跨度；6 列、13 行等推荐默认不得被当作合法值目录。拖拽、键盘与命令缩放逐整数单位变化。V2 自动迁移为 V3：旧双列的列位置与列跨度等比迁移到 12 列，已有有效整数行跨度保持；回归同时覆盖来源同步、预设往返、窄栏投影、Provider `layoutProjection`、组内/页面级矩形碰撞、投影不污染持久布局，以及 Classic / Nodes 2.0 的实际对齐。
