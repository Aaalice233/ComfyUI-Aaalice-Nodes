...
- 卡片操作网格回归至少覆盖当前 5 个操作的 205×48 横排、48×205 竖排、143×80 的 3×2 网格和 120×45 的更多菜单；每种布局必须完整显示全部入口且保持 28px 点击目标。更多菜单只列出当前来源真实可见的操作，收藏状态与禁用态同步；菜单选择仍执行原操作，卡片回收、换绑或 Surface 销毁时关闭菜单并且不恢复到已卸载焦点。
