...
- Discord 分享分别验证工作区侧栏底栏纸飞机、顶栏、隐藏三态及重载持久化；底栏 GitHub、Discord 社区链接和右侧固定按钮保持可用，两处右键迁移后只能保留一个可见分享入口，设置必须能恢复隐藏入口。首次点击在没有运行结果时仍先完成 Discord 授权与目标服务器成员检测；非成员显示配置的社区邀请，成员验证成功后才以 Toast 提示运行工作流。右键 Preview Any 后保存/加载仍能解析提示词来源；连续两次运行只展示后一次成功执行的去重图像，缩略图、文件名、分辨率、方向键和缺失提示词禁用态正确。
- 中继纯逻辑覆盖 Origin 白名单、OAuth state、会话 TTL、成员/角色失败、速率和大小上限、提示词代码块转义与长 Prompt 分段。真实 Discord OAuth、退出服务器后的再次拒绝、Webhook 图像与提示词显示、长 Prompt 多消息和撤销会话属于外部系统验收，不得用 mock 结果声明通过。
