...
- 连续滚动到至少第 5 个逻辑页后，浏览器页签隐藏/恢复、节点与 Dashboard 投影来回切换、Dashboard Surface 销毁重建以及改变容器宽度/列数，都必须保留同一 `source + postId` 卡片及其视口相对偏移，页码不得回到第 1 页；多个 Dashboard 根同时活动时只有用户正在交互的 owner 可以同步逻辑页，后台投影的重排不得覆盖页码；暂停投影收到多次图片自然尺寸修正时只记失效，重新激活后合并为一次 reflow。显式改变搜索上下文或跳页仍应清除旧锚点。
