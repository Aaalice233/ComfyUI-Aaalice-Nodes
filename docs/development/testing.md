...
文档变更还需检查 Markdown 相对链接、双语 README 标题结构、ADR 状态和 `AGENTS.md` 行数。
...
ParameterReceiver 前端改动至少覆盖：

| 路径 | Classic | Nodes 2.0 |
|---|:---:|:---:|
| 创建并绑定当前图中的面板 | ✓ | ✓ |
| 确认补齐 Set、创建折叠 Get 并连接真实 socket | ✓ | ✓ |
| 改名自动刷新；增删、重排和断线进入“需要同步” | ✓ | ✓ |
| 手动同步、下游连线按 Parameter Id 重绑 | ✓ | ✓ |
| 源面板缺失、KJNodes 缺失、解除和重新绑定 | ✓ | ✓ |
| 保存重载、复制、撤销与重做 | ✓ | ✓ |
