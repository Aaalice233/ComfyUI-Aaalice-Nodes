...
- Discord 分享分别验证侧栏、顶栏、隐藏三态及重载持久化；两处右键迁移后只能保留一个可见入口，设置必须能恢复隐藏入口。无运行结果显示 Toast；右键 Preview Any 后保存/加载仍能解析提示词来源；连续两次运行只展示后一次成功执行的去重图像，缩略图、文件名、分辨率、方向键和缺失提示词禁用态正确。
