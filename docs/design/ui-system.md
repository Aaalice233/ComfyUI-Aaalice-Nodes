...
普通模式和布局模式下，向下滚动到当前 Dashboard 页面底部都会自动进入后一页，向上滚动到顶部都会自动返回前一页；内容不足一屏时按滚动方向直接翻页。滚轮监听属于不参与页面位移动画的稳定舞台，每个侧栏根实例拥有独立状态。连续滚轮事件不排队，每 180 ms 最多切换一页，不按 delta 幅值猜测多页；停止后不追加翻页。指针所在的文本框、列表或其它嵌套滚动区能继续滚动时优先保留原生行为；页面内部已被原生滚动消费的事件只在下一帧检查最后一个越界方向，不重放历史输入。手动选页、首尾、工作区/工作流切换、搜索、布局模式或拖拽则同步取消当前手势。多个侧栏实例分别播放同一方向的页面过渡，不共享离场 DOM；同帧相反请求最终回到原 Page Id 时不播放同页伪过渡。边界翻页完成后只同步页码和页面内容，不额外展开悬浮表面；首尾页面保留普通的滚动边界反馈。
