...
普通模式和布局模式下，向下滚动到当前 Dashboard 页面底部都会自动进入后一页，向上滚动到顶部都会自动返回前一页；内容不足一屏时按滚动方向直接翻页。滚轮监听属于不参与页面位移动画的稳定舞台，每个侧栏根实例拥有独立状态。物理滚轮一个有强设备证据的离散事件只生成一个翻页意图；边界处的快速同向脉冲逐事件进入与 180 ms 页面过渡同频的串行队列，物理/精密反向脉冲替换旧方向，已消费的精密惯性也先取消相反物理队列，不按 delta 幅值猜测多页。精密输入以 240 ms 静默间隔划分手势，一次连续手势最多切换一页，惯性持续期间不得连续跨页。指针所在的文本框、列表或其它嵌套滚动区能继续滚动时优先保留原生行为并立即清除旧页面队列；页面内部已被原生滚动消费的事件只在下一帧保留最终越界方向，不重放整批历史输入。重挂后按实时 Page Id 继续队列，手动选页、首尾、工作区/工作流切换、搜索、布局模式或拖拽则同步清理不再有效的意图。多个侧栏实例分别播放同一方向的页面过渡，不共享离场 DOM；同帧相反请求最终回到原 Page Id 时不播放同页伪过渡。边界翻页完成后只同步页码和页面内容，不额外展开悬浮表面；首尾页面保留普通的滚动边界反馈。
