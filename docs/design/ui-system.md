...
Aaalice 工作区的页面内容滚轮和触控板惯性滚动只服务当前 Dashboard 页面；页面首尾、内容高度和滚动速度都不得隐式触发页面切换。右侧 Page Rail 是独立的显式导航表面：常态在 Dashboard body 中占用固定 38px 独立列显示圆点，不遮挡 Scroll Surface 或控件；指针进入后胶囊向内容侧展开为带页面名称的胶囊，展开后的透明命中区必须覆盖所有胶囊之间的间隙，只有离开整个展开包络才收回；键盘 `focus-visible` 也保持展开以支持导航，当前胶囊整体使用强调色光晕而不只突出圆点。只有 Rail 自身消费垂直滚轮、点击或键盘操作时才切换页面。Rail 必须阻止滚轮继续落入页面 Scroll Surface，页面 Scroll Surface 不得反向监听边界翻页。连续输入不建立延迟队列，交互结束后不追加旧页面请求。
...
普通模式和布局模式下，当前页面底部与顶部都保留普通的滚动边界反馈，内容不足一屏也不触发页面切换。页面切换只通过页眉左侧的页面按钮或右侧 Page Rail 完成：Rail 的点击、Arrow/PageUp/PageDown/Home/End 键盘操作和 Rail 内垂直滚轮都直接选择目标 Page Id，不把输入排入延迟队列；Ctrl/Meta 或横向占优滚轮不被 Rail 接管。多个侧栏实例分别维护 Rail、当前页光标和页面过渡，不共享离场 DOM；同帧相反请求最终回到原 Page Id 时不播放同页伪过渡。Rail 的悬停展开只改变其自身胶囊视图，不重建页面内容、抢占文本焦点或改变 Scroll Surface 的滚动位置。

Layout Group 在视图中呈现为一张 Composite Card：组名、组操作和成员共享一个外壳，成员常态只保留统一的低对比中性背景，不重复绘制完整边框和投影。布局模式中的瞬时选中是例外：组内成员与页面散项、分隔项使用同一层不改变尺寸的明亮选择轮廓，同时保留成员单侧状态条；Missing Binding 继续通过背景和单侧状态条表达。Seed 的执行后行为只在其模式按钮上用图标与低强度语义色呈现，避免重新制造常驻内层卡片边框；成员仍保持独立身份、网格占位、拖拽、缩放和绑定，不把组合视觉写入模型。
