...
- Nodes 2.0 的内部滚动遵循 ComfyUI 焦点式滚轮归属：滚动根可通过 `tabindex="0"` 进入焦点顺序并声明 `data-capture-wheel="true"`；当焦点位于该根或其后代时，宿主才把普通滚轮留给组件。允许“指针位于列表上直接滚动”的组件，应在目标阶段的普通 `wheel` 处理里仅补齐 `focus({ preventScroll: true })`，使焦点在事件冒泡到宿主前建立；若 `Ctrl` / `Meta` 正用于画布缩放，则不得抢焦点。
...
- 含内部滚动区的 Nodes 2.0 节点必须分别在 Standard / Legacy 导航模式验证：首次把指针移入未聚焦列表后滚动即可滚动内容；焦点位于列表或其后代时普通滚轮不移动或缩放画布；焦点在列表外时宿主行为保持原样；Standard 模式下 `Ctrl` / `Meta` + 滚轮仍缩放画布。静态契约同时断言捕获属性、可聚焦入口、焦点建立以及没有直接阻断或伪造滚轮事件。
