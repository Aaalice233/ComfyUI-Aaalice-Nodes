...
Dashboard V3 的规范布局使用 12 列、4px 行轨道和 2px 轨道间隔。控件 `columnSpan` 可取 3–12 的任意整数，`rowSpan` 可取不小于 13 的任意整数；6 列、13 行等只定义推荐起始尺寸，不构成合法值目录。拖拽时必须同时保留指针在卡片内的横向与纵向抓取偏移，使卡片跟随位置和吸附预览使用同一个逻辑锚点；手动双轴卡片的右下角拖柄、动态单轴卡片的对应边缘拖柄、方向键和布局命令都按整数网格单位调整，拖柄松手后通过一次命令提交并处理矩形碰撞，`Shift` 加速为两个单位。“整理布局”按稳定视觉顺序确定性紧凑排列矩形占位，重复执行保持幂等。侧栏过窄时投影成一列，允许继续调整高度，但不得用投影尺寸覆盖工作流中的规范宽度。Provider 可返回非持久的 `layoutProjection`（`rowSpan` / `columnSpan`）；页面和组内都使用通用投影重排后续占位，但 `data-drop-*` 和工作流中的规范布局不变。多选快捷操作条仅在存在选择项时显示并为内容区预留底部安全空间；未选择任何布局项时不得保留空白占位。Dashboard V2 自动迁移为 V3：旧双列的列位置与列跨度等比映射到 12 列，原有有效整数行跨度保持，不保留第二套布局算法。
...
Layout Group 在视图中呈现为一张 Composite Card：组名、组操作和成员共享一个外壳，成员只保留统一的低对比中性背景，不重复绘制完整边框和投影。成员的选中与 Missing Binding 通过背景强度和单侧状态条表达；Seed 的执行后行为只在其模式按钮上用图标与低强度语义色呈现，避免重新制造内层卡片边框；成员仍保持独立身份、网格占位、拖拽、缩放和绑定，不把组合视觉写入模型。
