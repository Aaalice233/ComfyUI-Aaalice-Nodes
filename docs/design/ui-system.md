...
Aaalice 工作区的页面内容滚轮和触控板惯性滚动只服务当前 Dashboard 页面；页面首尾、内容高度和滚动速度都不得隐式触发页面切换。右侧 Page Rail 是独立的显式导航表面：常态在 Dashboard body 中占用固定 38px 独立列显示圆点，不遮挡 Scroll Surface 或控件；收起态只有圆点簇中央的动态命中区响应指针，圆点上下两侧的空白不触发展开；指针进入后胶囊向内容侧展开为带页面名称的胶囊，展开后的透明命中区必须覆盖所有胶囊之间的间隙，只有离开整个展开包络才收回；键盘 `focus-visible` 也保持展开以支持导航，当前胶囊整体使用强调色光晕而不只突出圆点。Rail 只通过点击或键盘操作切换页面，不监听或消费滚轮；页面 Scroll Surface 不得反向监听边界翻页。连续输入不建立延迟队列，交互结束后不追加旧页面请求。
...
普通模式和布局模式下，当前页面底部与顶部都保留普通的滚动边界反馈，内容不足一屏也不触发页面切换。页面切换只通过页眉左侧的页面按钮或右侧 Page Rail 完成：Rail 的点击和 Arrow/PageUp/PageDown/Home/End 键盘操作直接选择目标 Page Id；Rail 不监听滚轮，滚轮不会触发页面切换。多个侧栏实例分别维护 Rail、当前页光标和页面过渡，不共享离场 DOM；同帧相反请求最终回到原 Page Id 时不播放同页伪过渡。Rail 的悬停展开只改变其自身胶囊视图，不重建页面内容、抢占文本焦点或改变 Scroll Surface 的滚动位置。
