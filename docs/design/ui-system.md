...
普通模式和布局模式下，向下滚动到当前 Dashboard 页面底部都会自动进入后一页，向上滚动到顶部都会自动返回前一页；内容不足一屏时按滚动方向直接翻页。每个独立滚轮脉冲或触控板手势最多切换一页；滚动输入短暂停顿后必须立即重新响应，不能用会被离散滚轮持续延长的长冷却阻塞后续翻页，触控板惯性持续期间仍不得连续跨页。边界翻页完成后只同步页码和页面内容，不额外展开悬浮表面或阻塞下一次独立输入。搜索期间和正在拖拽布局项时不启用边界翻页，避免筛选或未完成手势被切页打断；首尾页面保留普通的滚动边界反馈。
