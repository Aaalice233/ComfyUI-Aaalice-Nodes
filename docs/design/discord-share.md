...
| 最新运行图像与提示词 | 浏览器内存中的最后一次含图像执行快照；分享 Dialog 内的临时编辑草稿 | 页面会话；编辑草稿仅存活于当前 Dialog |
...
成功执行完成后从 `/history/{prompt_id}` 读取完整 outputs；失败或中断时直接使用本次
`executed` 事件已经收到的输出。只要失败执行已产出图片就替换上一快照，完全没有图片时才
保留上一次结果。图像按执行输出顺序去重，提示词按限定执行 Id 反查绑定节点。最新运行数据、
选中缩略图、图片尺寸、Dialog 状态和发送前编辑的提示词草稿不得写入工作流；编辑只影响本次分享。
...
3. 验证成功但没有可分享图像时只显示原生 Toast，不打开空相册；用户下次点击
   无需再次授权。
4. 已验证且存在结果时打开最新运行相册。相册默认选择像素面积最大的结果；面积相同或
   尚未完成尺寸解码时选择执行输出顺序更晚的一张。左侧媒体区以可缩放、拖动和双击还原的大图为
...
6. Footer 在发送按钮旁提供频道多选，至少保留一个频道；触发器固定显示向下箭头与
   “可多选”提示，展开时箭头旋转，不能要求用户先点击才发现多选能力。客户端只取得稳定 Target Id、
