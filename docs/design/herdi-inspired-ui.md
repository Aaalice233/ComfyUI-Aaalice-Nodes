...
- ParameterPanel 节点面顶部两个角保持直角，底部两个角沿用节点圆角；Classic Canvas 与 Nodes 2.0 DOM 壳必须保持一致。
- 节点最小宽度约 370 graph units，参数区左侧保留约 10px 安全边距，控件内容为输出列留出约 53px，并在控件与输出文本之间保留明确 gutter；普通参数行使用约 58px（标签/数值行 + 32px 控件 + 间隙），多行文本和分隔区按内容增加高度，节点高度取实际内容与输出行的较大值，不能让 socket 被截断或覆盖。
...
- 可输入字段使用由 `--comfy-input-bg`、画布层和边框色派生的深色内嵌表面，通过明确但克制的明暗差、实线轮廓和内阴影与节点底板分层；默认态必须比节点底板更深，hover 和编辑态才适度提亮。只读数值与滑条底板保持扁平，避免过度框选。挂载到页面根部的临时数字编辑器必须复制节点已解析的主题色，使用不透明的高对比编辑表面，并隐藏原生微调箭头与底层静态数值。
...
- 标题栏只显示“编辑参数”和参数数量，不重复展示节点名称、产品类型或说明文案。
- 宽屏保持参数列表 + inspector 双栏；窄屏改为上下布局。
- 左栏顶部使用简短的“参数”标题和排序提示；参数类型下拉框与纯图标新增按钮组成一个连续控件，统一外轮廓、焦点反馈和至少 40px 命中高度。列表在剩余空间独立滚动；参数行只负责身份、选择、重排、复制和删除，次要操作默认弱化，避免形成密集按钮墙。
- 左侧选中态已经表达当前参数身份，inspector 不重复设置参数标题栏；右侧直接从字段标签开始，两列字段顶部对齐，不显示“基础信息、取值规则、选项与行为”等重复分区标题或横线。分区名称仅作为 `aria-label` 保留语义，不套用卡片边框、圆角或阴影。
- 编辑器高度应随视口受限且避免无意义的大面积空白；数值规则在宽屏同排展示，窄屏自动堆叠。
- Markdown 说明框在宽屏双栏中垂直填满 inspector 的可用高度，长内容在输入框内部滚动且不允许拖拽破坏布局；窄屏使用约 280px 的稳定高度。
- 编辑器正文和控件以约 13px、`1.45–1.6` 行高为基准，辅助文字不得压缩到难以阅读的尺寸；使用字重建立层级，避免所有标题同时重粗。
- 数值规则按实际字段数分栏：Seed 的最小值/最大值二等分，Slider 的最小值/最大值/步长三等分；标签与生成后行为必须显示本地化文案，协议值保持英文稳定键。数字输入启用 tabular numerals，规则组之间保留更明确的垂直间距。
- footer 固定承载草稿状态、校验错误、取消和保存；断线确认与原子保存语义不得改变。
