...
registerWidgetControlAdapter({
	...
	describe({ node, widget }) {
		...
		return {
			...
			kind: "resolution",
			presetEditor: { create: control.createPresetEditor },
...
registerWidgetControlAdapter({
	...
	describe({ node, widget }) {
		...
		return {
			...
			rowSpan: 90,
			minRowSpan: 50,
			options: { createSidebarControl: () => runtime.createSidebarControl(), getCapabilities: () => runtime.getCapabilities(), collectionOptions: (source) => runtime.collectionOptions(source) },
